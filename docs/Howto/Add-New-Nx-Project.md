...
1. **Generate the project**
   ```bash
   pnpm exec nx g @nrwl/node:library libs/my-new-lib --directory=packages --unitTestRunner=vitest
   ```
...
3. **Add targets**
   ...
   - Use Nx cacheable operations so distributed agents can execute them; run `pnpm exec nx show project my-new-lib` to confirm.
...
5. **Validate**
   ```bash
   pnpm exec nx test my-new-lib
   pnpm exec nx run-many --target=build --projects=my-new-lib
