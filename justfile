...
nx-init:
	bash -lc 'set -euo pipefail; if [ "${HOMELAB:-0}" != "1" ]; then echo "Refusing nx-init: HOMELAB != 1" >&2; exit 1; fi; echo "Installing Bun dependencies..."; bun install; echo "Validating nx.json schema..."; if ! command -v jq >/dev/null 2>&1; then echo "WARN: jq not installed, skipping JSON validation" >&2; else jq empty nx.json || { echo "ERROR: nx.json is invalid JSON" >&2; exit 1; }; fi; echo "Nx workspace initialized successfully"'
...
nx-cache-server-start:
	bash -lc 'set -euo pipefail; if [ "${HOMELAB:-0}" != "1" ]; then echo "Refusing nx-cache-server-start: HOMELAB != 1" >&2; exit 1; fi; if ! command -v tailscale >/dev/null 2>&1; then echo "ERROR: Tailscale CLI not found" >&2; exit 2; fi; TAILSCALE_IP=$(tailscale ip -4 2>/dev/null || echo "127.0.0.1"); CACHE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/nx-cache"; mkdir -p "$CACHE_DIR"; echo "Starting Nx cache HTTP server on $TAILSCALE_IP:3333..."; echo "Cache directory: $CACHE_DIR"; if command -v bunx >/dev/null 2>&1; then nohup bunx http-server "$CACHE_DIR" --port 3333 --cors --host "$TAILSCALE_IP" > "$HOME/.local/state/nx-cache-server.log" 2>&1 & echo $! > "$HOME/.local/state/nx-cache-server.pid"; echo "Cache server started (PID $(cat "$HOME/.local/state/nx-cache-server.pid"))"; echo "Logs: $HOME/.local/state/nx-cache-server.log"; else echo "ERROR: bunx not found (required for http-server)" >&2; exit 2; fi'
