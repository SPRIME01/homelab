...
Accepts --full-conf and --answers JSON strings generated by copier for deeper validation.
"""
import argparse
import json
import re
import sys
# pathlib.Path is not used here; keep imports minimal
...
def check_answers(answers: dict):
    ...
    if 'npm_scope' in answers and not is_npm_scope(answers.get('npm_scope', '')):
        die(f"npm_scope must be empty or start with @ and contain lowercase alnum or -/_; got: {answers.get('npm_scope')}")
