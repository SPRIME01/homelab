...
def _setup_k3s_wsl2_service(self) -> None:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            self.run_command(
                ["sudo", "systemctl", "enable", "k3s"], "Enable K3s service"
            )
...
"""
            ...
            subprocess.run(
                ["sudo", "mv", "/tmp/k3s-start.sh", "/usr/local/bin/k3s-start.sh"]
            )
...
def _setup_k3s_kubeconfig(self, wsl2_ip: str, windows_host_ip: str) -> bool:
    ...
    try:
        ...
        # Copy K3s kubeconfig
        subprocess.run(
            ["sudo", "cp", "/etc/rancher/k3s/k3s.yaml", str(kube_dir / "config")]
        )
        ...
        external_config = content.replace(
            f"https://{wsl2_ip}:6443", f"https://{windows_host_ip}:6443"
        )
        ...
        success, output = self.run_command(
            ["kubectl", "get", "nodes"], "kubectl cluster access test"
        )
        if success:
            self.installed_components.append(
                f"K3s cluster: {output.strip().split()[0]} node ready"
            )
            self.log_message(f"✅ K3s accessible on WSL2 IP: {wsl2_ip}:6443", "SUCCESS")
            self.log_message(
                f"✅ K3s accessible on Windows IP: {windows_host_ip}:6443", "SUCCESS"
            )
            return True
        else:
